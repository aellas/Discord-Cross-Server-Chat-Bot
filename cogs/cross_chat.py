    ...
    async def on_message(self, message):
        ...
        if message.channel.id in channel_ids:
            ...
            if any(word in message.content.lower() for word in swear_words) or any(role in message.content for role in mentioned_roles):
                await message.channel.send("Warning: Your message contains a swear word or a role mention, message not sent.")
                ...
                if self.user_warnings[message.author.id] >= 5:
                    await message.author.ban(reason="Exceeded warning limit for advertising bot.")
            else:
                ...
                for channel_id in channel_ids:
                    if channel_id != message.channel.id:
                        ...
                        if channel:
                            ...
                            embed = nextcord.Embed(description=f"Sent by **{message.author.name}** (`{message.author.id}`) from **{message.guild.name}** ", color=0xffffff)
